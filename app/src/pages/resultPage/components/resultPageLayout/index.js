          ...
          <div>
            {isError ? <p className={styles.error}>{error.response}</p> :
              <>
                ...
                {info[0].meanings[0]?.definitions[0].example ? <p className={styles.description}>Example: {info[0].meanings[0]?.definitions[0].example}</p> : null}
                {info[0].meanings[1]?.partOfSpeech ? <p className={styles.description}>Part Of Speech: {info[0].meanings[1]?.partOfSpeech}</p> : null}
                {info[0].meanings[1]?.definitions[0].definition ? <p className={styles.description}>Definition: {info[0].meanings[1]?.definitions[0].definition}</p> : null}
                {info[0].meanings[1]?.definitions[0].example ? <p className={styles.description}>Example: {info[0].meanings[1]?.definitions[0].example}</p> : null}
                {info[0].meanings[1]?.definitions[0].synonyms.join(', ') ? <p className={styles.description}>Synonyms: {info[0].meanings[1]?.definitions[0].synonyms.join(', ')}</p> : null}
